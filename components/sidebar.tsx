...
export default function Sidebar() {
  ...
  const { locations, selectedLocationIds, setSelectedLocationIds, allSelected } = useLocationContext();
    ...
    <aside className="hidden md:flex flex-col w-[220px] shrink-0 bg-white border-r border-slate-200/70 min-h-screen sticky top-0 h-screen">
      ...
      {locations.length > 1 && (
        <div className="px-3 pb-3 pt-1 border-b border-slate-100">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 px-1">Locations</p>
          <div className="flex flex-col gap-0.5">
            <button
              onClick={() => setSelectedLocationIds(locations.map(l => l.id))}
              className={`flex items-center gap-2 w-full px-2.5 py-1.5 rounded-lg text-[12px] font-semibold transition-colors text-left ${
                allSelected ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-50 hover:text-slate-700"
              }`}
            >
            ...
            {locations.map(loc => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 w-full px-2.5 py-1.5 rounded-lg text-[12px] font-semibold transition-colors text-left ${
                    selected ? "bg-slate-100 text-slate-800 border border-slate-200" : "text-slate-400 hover:bg-slate-50 hover:text-slate-600"
                  }`}
                >
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${selected ? "bg-emerald-400" : "bg-slate-200"}`} />
                  {loc.name}
                </button>
              );
            })}
