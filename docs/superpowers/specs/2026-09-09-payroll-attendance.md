...
- **$500 deduction flag:** automatically shown on payroll report when driver's `terminationType = "notice"` (quit without completing notice or didn't give notice). If `terminationType = "fired"` — no deduction, company's decision.
...
- Any termination deduction flag: "−$500" badge if terminationType="notice"
