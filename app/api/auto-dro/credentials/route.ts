...
export async function POST(req: Request) {
  ...
  await Promise.all([
    db.insert(settings).values({ organizationId: orgId, key: "dro_username", value: username })
      .onConflictDoUpdate({ target: [settings.organizationId, settings.key], set: { value: username } }),
    db.insert(settings).values({ organizationId: orgId, key: "dro_password", value: password })
...
export async function GET() {
  ...
  const rows = await db.select().from(settings)
    .where(and(eq(settings.organizationId, orgId), eq(settings.key, "dro_username")));
